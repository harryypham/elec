...
void setup_robot(struct Robot *robot)
{
    ...
    robot->turn_left = 0;
    robot->turn_right = 0;
    robot->big_wall_state = 0;
    robot->state_while_turn = 0;
    robot->big_wall = 0;
    robot->check_big_wall = 0;
...
{
    if (robot->check_big_wall == 0)
    {
        if (robot->big_wall == 0 && left_sensor == 0 && right_sensor == 0)
        {
            robot->big_wall = 1;
        }
        robot->check_big_wall = 1;
    }

    switch (robot->big_wall)
    {
    case 1:

        robot->big_wall_state = 1;
        robot->turn_left = 1;
        robot->big_wall++;
        break;
    case 2:
        ...
        break;
    case 3:
        if (robot->state_while_turn == 1)
        {
            robot->big_wall_state = 0;
        }
    }
    if (robot->big_wall_state != 0)
    {
        switch (robot->big_wall_state)
        {
        case 1:
            if (robot->turn_left == 1)
            {
    ...
    else if (left_sensor < 1 || right_sensor > 3)
    {
        robot->direction = LEFT;
    }
    ...
    else if (front_centre_sensor == 0 && left_sensor <= 3)
    {
